    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (IsSortedByAscending(numbers))
            {
                ...
                numbers = numbers.Select(i => i = 133).ToArray();
            }
            else if (IsSortedByDescending(numbers))
            {
                Console.WriteLine("Массив отсортирован по убыванию, значения заменены на противоположные");
                numbers = numbers.Select(i => i * -1).ToArray();
            }
            else
            {
                Console.WriteLine("Массив не отсортирован, действий не требуется");
            }
            PrintArray(numbers);
        }
    }
